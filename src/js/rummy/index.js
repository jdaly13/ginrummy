import { cardArray, cardSuits, discard, translateX } from '../constants';
import {one_suit, cardValues, createDeckOfCards, shuffle} from '../utils';
import { flipNewDeck, reshuffle, dealCards } from '../actions';
...
oneTimeEvents.createarrayofcards = function (callcreatehtmldeck) {
    var second_key = '',
        ...
        arr = [],
        key;
    for (key in this.deckofcards) { // we use for in loop because it loops through prototype which we want
        ...
        for (second_key in suit) { //second_key is ace, two, three, etc    
            if (!callcreatehtmldeck) arr.push(second_key + ' of ' + key + ' ' + suit[second_key]); //e.g ace of hearts 1
        }
    }
...
oneTimeEvents.loopthroughdiv = function () {
    ...
    var space;
    var last;
    Array.from(this.DOMdeck.querySelectorAll('[data-side="front"]')).forEach((ele, index)=> {
        ...
        Object.assign(ele.parentElement.style, {'transform': 'translate(' + (1 + index) + 'px,' + (1 + index) + 'px)', zIndex: 1 + index});
        last = space.pop();
        ele.innerHTML = "<p>" + space.join(" ") + "</p><a href='#'>" + discard + "</a><span>" + last + "</span>";
    });
};
...
oneTimeEvents.flipNewDeck = function () {
    var comp_player = this.DOMdeck.querySelectorAll('.comp_player'),
        ...
        fp_pos = first_player[9].getBoundingClientRect(),
    ...
    Array.from(first_player).forEach((ele, index) => {
        ...
        ele.addEventListener('transitionend', (e)=> {
            ...
            var sortable = Sortable.create(this.DOMplayerArea);
            if (index === 0) {
...
oneTimeEvents.dealfirstcard = function () {
    ...
    deckLastZChild.classList.add('showdacard', 'delt');

    deckLastZChild.addEventListener('transitionend', (ele) => {
