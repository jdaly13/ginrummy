...
RUMMY.prototype = {
  discard: function($discard, $obj) {
    ...
    if (
      $lastChild.hasClass('showdacard') ||
      $lastChild.hasClass('player') ||
      $lastChild.hasClass('comp_player')
    ) {
      $obj
        .appendTo($deck)
        .css({
  ...
  startTheProcess: function(withSuit, playerOne) {
    ...
    if (playerOne === 'topCard') {
      //does computer player take top Card
  ...
  beginningOfTheEnd: function(playerOne, total, objectsGalore) {
    ...
    if (playerOne === 'computer' || playerOne === 'findFirstPlayerScore') {
      ...
      showCardObj = this.showYourCardsJoshua(
        firstplayerScore ? compPlayer.deck : objectsGalore
      );
      this.manipulateTheDom(showCardObj); // manipulate the DOM
      deadWoodArray = this.laySomeDeadwood(
        firstplayerScore ? true : false,
        objectsGalore
      );
      console.log(deadWoodArray);
      deadWoodMatch = this.compareDeadwood(
        deadWoodArray,
        firstplayerScore ? compPlayer.deck : player.deck,
        firstplayerScore ? 'joshua' : false
      );
      ...
      if (deadWoodMatch) {
        ...
        finalScore =
          finalScore < 0 ? 0 - whatToMinus : finalScore - whatToMinus;
      }
  ...
  helpfulHints: function(param) {
    ...
    var secondString =
      'Sort your cards by either suit or match. Drag each card with your mouse. When you are done sorting, either take the top card from the discard pile or click take from deck button';
    var thirdString =
      'Before you discard any of your cards you have the option to knock (end the game) - Any remaining cards that are not part of a valid combination are considered deadwood, \n\
    ...
    if (param === 'first') {
      $html = $(
        '<div class="first_bubble bubble">Click below to start game</div>'
      );
...
rummy.iframeptext =
  'You will be playing the computer (aka Joshua), with each hand you the player will always go first. Whomever reaches 100 points first will be crowned winner of the game. Below are instructions to the game, you should only see this intro screen once.';
...
rummy.$intoIframe = $(
  '<section id="intro"><h1>' +
    ...
    '</h1><p>' +
    rummy.iframeptext +
    '</p><audio id="shallWePlayAGame" controls="controls"><source src="playgame.wav" type="audio/wav" /></audio><iframe width="1000" height="600" src="http://www.pagat.com/rummy/ginrummy.html"></iframe></section>'
);
  ...
  for (key in this.deckofcards) {
    ...
    for (second_key in suit) {
      ...
      if (!callcreatehtmldeck)
        this.whole_deck.push(
          second_key + ' of ' + key + ' ' + suit[second_key]
        ); //e.g ace of hearts 1
    }
  }
...
oneTimeEvents.fillinmaincontent = function() {
  ...
  var buttons =
    '<article><button id="deal">dealcards</button> <button disabled id="reshuffle">reshuffle</button><button id="takeCard" class="hide">Take from deck</button> <button id="knock" class="hide">Knock</button> </article>';
  ...
  if (whichPlayer === 'player') {
    ...
    $object
      ...
      .one(transitionEndEvent, function(event) {
        ...
        $(obj)
          .appendTo(that.$area)
          .removeAttr('style'); //.removeStyle('transform');
...
player.takeCardEvent = function($deck, $takefromdeckbutton) {
  ...
  $takefromdeckbutton.on('click', function() {
    ...
    var $topFromDeck =
      $deck.find('.delt:first').prev() || $deck.find(':last-child').prev();
    $topFromDeck
      .addClass('player temp speedUpAnimation')
      .css({
...
player.flipCards = function(which_one, $object) {
  ...
  if (which_one === 'top_card') {
    ...
    $object
      ...
      .one(transitionEndEvent, function() {
        $object
          .removeClass('showdacard flipit temp')
          .addClass('flipchild')
          .prependTo(that.$area)
          .removeAttr('style');
        that.playerHover($object, 'one');
...
compPlayer.takeNextCard = function() {
  ...
  var lastInHandLeftPos = (this.lastCardPos = parseInt(
    $compArea.children(':first-child').css('left')
  ));
  console.log(this.index);
  if (this.index === 1) {
    this.lastInDeck = lastInHandLeftPos;
  }
  ...
  var takeTopCard = this.preFilteringOfCreateArray(
    $compArea.children(),
    'topCard'
  );
  var $element = !!takeTopCard ? $topCardShown : $lastInDeck;
  $element
    .removeClass('flipchild player speedUpAnimation')
    .addClass('comp_player temp')
    .css({
      left: pos,
      top: '-185px',
      zIndex: zIndex
    ...
    .one(transitionEndEvent, function() {
      $(this)
        .prependTo($compArea)
        .removeClass('flipchild');
      var $selfAndSiblings = $(this)
        .siblings()
        .andSelf();
